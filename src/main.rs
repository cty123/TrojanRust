...
async fn main() -> Result<()> {
    ...
    info!("Starting {} server to accept inbound traffic", inbound_config.mode);
    match inbound_config.mode {
        trojan_rust::config::base::InboundMode::TCP => {
            tcp::server::start(inbound_config, outbound_config).await?;
        }
        trojan_rust::config::base::InboundMode::GRPC => {
            grpc::server::start(inbound_config, outbound_config).await?;
        }
        trojan_rust::config::base::InboundMode::QUIC => todo!(),
    }
