...
const REQUEST_HEADER_SIZE: usize = 1 + 16 + 1 + 1 + 2 + 1;
const INDEX_VERSION: usize = 0;
const INDEX_UUID: usize = 1;
const INDEX_ADDON: usize = INDEX_UUID + 16;
const INDEX_COMMAND: usize = INDEX_ADDON + 1;
        ...
        IO: AsyncReadExt + AsyncWriteExt + Unpin
{
    ...
    // Extract the UUID
    let uuid: [u8; 16] = buf[INDEX_UUID..INDEX_ADDON].try_into().expect("Buffer has incorrect size");
